...
def check_compression_header(data):
    """ Check the zlib compression header for correct method and info. """
    if len(data) < 2:
        return False, "Data too short to check compression header"
    
    cmf = data[0]
    method = cmf & 0x0F  # Compression method
    cinfo = cmf >> 4     # Compression info
    
    # Check method is 8 (DEFLATE)
    # Check cinfo is 7 (which implies a 32K window size)
    if method != 8 or cinfo != 7:
        return False, f"Unexpected compression method or window size: method={method}, cinfo={cinfo}"
    
    return True, "Compression method and window size are correct"
    ...
    with open(filename, 'rb') as file:
        ...
        while pos < len(content):
            ...
            if chunk_type == b'IDAT':
                ...
                # Check compression method and info
                valid_compression, msg = check_compression_header(chunk_data)
                print(msg)
                if not valid_compression:
                    print("Error: Invalid compression method or window size.")
...
read_png_idat_chunks('current.png')
